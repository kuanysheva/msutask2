...
# #LBL4: Функция для тренировки модели с добавлением автоматического сохранения и аугментации данных
def train_model_with_callbacks(model, train_data, val_data, epochs=10, batch_size=32):
    ...
    train_generator = datagen.flow(train_frames, {'classification': train_labels, 'regression': train_coords}, batch_size=batch_size)

    checkpoint = ModelCheckpoint('model_checkpoint.h5', save_best_only=True, monitor='val_loss', mode='min')

    history = model.fit(train_generator, validation_data=val_dataset, epochs=epochs, callbacks=[checkpoint])

    # #LBL5: Построение графиков
...
# #LBL6: Функция для кросс-валидации
def cross_validate_model(model, data, n_splits=5):
    kfold = KFold(n_splits=n_splits, shuffle=True)
    frames, labels, coords = data
    for train_idx, val_idx in kfold.split(frames):
        train_data = (frames[train_idx], labels[train_idx], coords[train_idx])
        val_data = (frames[val_idx], labels[val_idx], coords[val_idx])
        history = train_model_with_callbacks(model, train_data, val_data)
    return history

# #LBL7: Функция для расчета Simple Ball Tracking Accuracy (SiBaTrAcc)
...
# #LBL8: Подготовка данных для оценки
...
# #LBL11: Кросс-валидация модели
history = cross_validate_model(model, train_data)
